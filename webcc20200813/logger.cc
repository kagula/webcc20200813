...
static FILE* FOpen(const boost::filesystem::path& path, bool overwrite) {
#if (defined(_WIN32) || defined(_WIN64))
...
struct Logger {
  ...
  void Init(const boost::filesystem::path& path, int _modes) {
    modes = _modes;
...
static boost::filesystem::path InitLogPath(const boost::filesystem::path& dir) {
  if (dir.empty()) {
    return boost::filesystem::current_path() / WEBCC_LOG_FILE_NAME;
  }

  if (!boost::filesystem::exists(dir) ||
      !boost::filesystem::is_directory(dir)) {
    boost::system::error_code ec;
    if (!boost::filesystem::create_directories(dir, ec) || ec) {
