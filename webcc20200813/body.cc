...
FileBody::FileBody(const boost::filesystem::path& path, std::size_t chunk_size)
...
FileBody::FileBody(const boost::filesystem::path& path, bool auto_delete)
...
FileBody::~FileBody() {
  if (auto_delete_ && !path_.empty()) {
    ...
    boost::filesystem::remove(path_.c_str(), ec);
    if (ec) {
...
void FileBody::InitPayload() {
  ...
  ifstream_.open(path_.c_str(), std::ios::binary);
...
Payload FileBody::NextPayload(bool /*free_previous*/) {
  if (ifstream_.read(&chunk_[0], chunk_.size()).gcount() > 0) {
    return { boost::asio::buffer(chunk_.data(),
                                 (std::size_t)ifstream_.gcount())
    };
  }
  return {};
}
...
bool FileBody::Move(const boost::filesystem::path& new_path) {
  if (path_ == new_path) {
    return false;
  }
  ...
  boost::system::error_code ec;
  boost::filesystem::rename(path_, new_path, ec);
